...
export default async function ProblemStatementsLayout({ children }: { children: ReactNode }) {
  ...
  if (userId) {
    ...
    if (!isSuperAdmin) {
      announcementQuery = announcementQuery.eq('admin_only', false);
    }
    const { data: latestAnnouncement } = await announcementQuery.maybeSingle();
