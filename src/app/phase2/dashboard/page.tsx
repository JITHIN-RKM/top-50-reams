...
import { Phase2PDFSubmission } from '@/components/dashboard/Phase2PDFSubmission';
import { Phase2ProblemStatementSection } from '@/components/dashboard/Phase2ProblemStatementSection';
...
export const metadata = {
  title: 'Phase 2 Dashboard — OUCE SIH 2026',
  description: 'Phase 2 registered team dashboard with PDF submission.',
};

export default async function Phase2DashboardPage() {
  ...
  if (!user.team_id) redirect('/phase2');

  const { data: team } = await supabase
    .from('teams')
    .select('id, name, leader_id, status, ps1_id, ps2_id')
    .eq('id', user.team_id)
    .maybeSingle();

  if (!team) redirect('/phase2');
  ...
  if (!registration) redirect('/phase2');
  ...
  const { data: files } = await supabase.storage.from('phase2_pdfs').list('', {
    search: `${team.id}.pdf`,
  });
  ...
  return (
    <div className="min-h-screen bg-sih-gray font-body text-sih-dark">
      ...
      <div className="max-w-5xl mx-auto px-4 md:px-8 py-10 md:py-16 space-y-8">
        {/* Success Banner */}
        <div className="bg-gradient-to-br from-blue-50 to-indigo-50 border-2 border-blue-200 p-8 md:p-10 relative overflow-hidden">
          <div className="absolute top-4 right-4 grid grid-cols-3 gap-1.5 opacity-30">
            {[...Array(9)].map((_, i) => (
              <div key={i} className="w-1.5 h-1.5 rounded-full bg-sih-blue" />
            ))}
          </div>

          <div className="flex items-start gap-4">
            <div className="w-14 h-14 bg-blue-100 border-2 border-blue-200 flex items-center justify-center flex-shrink-0">
              <PartyPopper className="w-7 h-7 text-sih-blue" />
            </div>
            <div>
              <h1 className="heading-display text-2xl md:text-3xl text-blue-900 mb-2">
                Successfully Registered for Phase 2!
              </h1>
              <p className="text-blue-700 text-base md:text-lg font-medium leading-relaxed">
                Team <strong>{team.name}</strong> is now registered for the Phase 2 Internal Hackathon.
                Submit your presentation PDF below. You can upload or update up until the event day (16 Sep).
              </p>
              <p className="text-xs text-blue-600 mt-3 font-bold uppercase tracking-widest">
        ...
        <Phase2ProblemStatementSection team={team} isLeader={isLeader} />
        ...
        <div className="bg-white border-2 border-gray-100 p-6 md:p-8">
          ...
          <div className="mt-4 flex items-center gap-2">
            ...
            <p className="text-xs text-gray-500 font-medium">
              {members?.length || 0}/6 members · Phase 2 registered
            </p>
          </div>
        </div>

        {/* PDF Submission */}
        <Phase2PDFSubmission teamId={team.id} userId={userId!} isLeader={isLeader} initialHasUploaded={hasUploaded} />
      </div>
    </div>
  );
}
